...
const subItems: Record<CategoryKey, string[]> = {
  ...
  academico: [
    "Modelo Educativo VASE",
    "Metodología CLIL",
    "Bachillerato Internacional IB",
    "Bachillerato General BGU",
    "Educación Básica",
  ],
  admisiones: [
    "Proceso de Admisión",
    ...
    <>
      ...
      >
        <nav className="bg-white h-[80px] flex items-center justify-between px-10 md:px-16">
          ...
          <div className="hidden md:flex items-center gap-7">
            ...
            <button
              aria-label="Buscar"
              className="text-[#1A2B4A] hover:opacity-60 transition-opacity"
            >
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            {/* Panel izquierdo — foto campus (desktop only) */}
            <motion.div
            ...
            {/* Panel derecho */}
            <motion.div
              className="flex-1 bg-[#0F1E30] flex flex-col overflow-y-auto"
              initial={{ x: 40, opacity: 0 }}
              animate={{ x: 0, opacity: 1 }}
              exit={{ x: 40, opacity: 0 }}
              transition={{ duration: 0.4, ease }}
            >
              {/* Top bar */}
              <div className="flex items-center justify-between px-16 h-[80px] flex-shrink-0">
              ...
              {/* Nav area — 2 columnas */}
              <div className="flex flex-1 gap-16 px-16 pt-10 pb-0">
                ...
                <div className="w-[260px] flex-shrink-0 flex flex-col gap-1">
                  ...
                  {categories.map((cat, i) => (
                    <motion.button
                      ...
                      className="flex items-center justify-between py-4 text-left w-full group"
              ...
              {/* Divisor horizontal */}
              <div className="mx-16 h-[1px] bg-white/10 mt-8" />

              {/* CTA row */}
              <div className="flex items-center justify-between px-16 py-8 flex-shrink-0">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-3">
                    <a
                      href="#"
                      className="bg-[#9e1915] text-white text-[13px] font-bold tracking-[0.5px] px-6 py-3 rounded hover:bg-[#7d140f] transition-colors"
                      onClick={() => setMenuOpen(false)}
                    >
                      Solicitar Admisión
                    </a>
                    <a
                      href="#"
                      className="border border-white/30 text-white/80 text-[13px] px-6 py-3 rounded hover:bg-white/10 transition-colors"
                      onClick={() => setMenuOpen(false)}
                    >
                      Portal Familiar
                    </a>
