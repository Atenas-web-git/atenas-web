...
export type RecSlug = "deportivos" | "academicos";

const ease: [number, number, number, number] = [0.25, 0.1, 0.25, 1];

const ITEMS: { slug: RecSlug; label: string }[] = [
  { slug: "deportivos", label: "Deportivos" },
  { slug: "academicos", label: "Académicos" },
];

export function NavReconocimientos({ current }: { current: RecSlug }) {
  return (
    <motion.nav
      ...
      <div className="flex items-stretch h-full px-4 md:px-[160px]" style={{ minWidth: "max-content" }}>
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.slug}
              href={`/reconocimientos/${item.slug}`}
