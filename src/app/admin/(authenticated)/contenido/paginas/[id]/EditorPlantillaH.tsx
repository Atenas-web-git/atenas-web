    ...
    <form action={action} className="flex flex-col gap-5">
      ...
      >
        <label className="flex items-start gap-3 cursor-pointer" style={{ marginBottom: 18 }}>
          ...
          />
          <span style={{ fontSize: 14, color: "#1A2B4A", lineHeight: 1.5 }}>
...
function HeroEditor({ hero, setHero, prefix }: { hero: Hero; setHero: (h: Hero) => void; prefix: string }) {
  ...
  return (
    <Card title="Bloque 1 — Hero" subtitle="Cabecera con collage flotante de 3 fotos, badge rojo con número grande y chips de niveles.">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Field label="Badge rojo flotante — número/valor" hint="Texto grande del badge (ej. 5).">
          <input type="text" value={hero.floatingBadgeValue} onChange={(e) => set({ floatingBadgeValue: e.target.value })} style={inputStyle} />
        </Field>
        <Field label="Badge rojo flotante — etiqueta" hint="Texto pequeño debajo (ej. NIVELES EDUCATIVOS).">
...
function NivelesEditor({ niveles, setNiveles, prefix }: { niveles: Niveles; setNiveles: (n: Niveles) => void; prefix: string }) {
  ...
  return (
    <Card title="Bloque 2 — Niveles educativos" subtitle="5 cards en fila desktop con número, título, edades, descripción, métodos (chips) y badge opcional.">
      ...
      <Field label="Badge rojo sobre el collage" hint="Aparece flotante sobre el collage de fotos del header (ej. IB ACREDITADO ★).">
        ...
        {niveles.items.map((it, i) => (
          <div key={i} className="flex flex-col gap-3 p-4" style={panelStyle}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Field label="Badge (opcional)" hint='Etiqueta roja destacada (ej. ÚNICO IB EN AMBATO).'>
            ...
            <Field label="Destacado">
              <label className="flex items-center gap-2" style={{ height: 38 }}>
                ...
                <span style={{ fontSize: 13, color: "#1A2B4A" }}>Card con fondo navy + borde rojo</span>
              </label>
            </Field>
          </div>
        ))}
      ...
      <div className="flex flex-col gap-3">
        {met.cards.map((c, i) => (
          <div key={i} className="flex flex-col gap-3 p-4" style={panelStyle}>
            ...
            <Field label="Alcance (scope)" hint="Línea pequeña roja (ej. Educación Inicial).">
            ...
            <Field label="Estilo dark">
              <label className="flex items-center gap-2" style={{ height: 38 }}>
                ...
                <span style={{ fontSize: 13, color: "#1A2B4A" }}>Tarjeta con fondo navy oscuro</span>
              </label>
            </Field>
          </div>
        ))}
...
function CTAEditor({ cta, setCta, prefix }: { cta: CTA; setCta: (c: CTA) => void; prefix: string }) {
  ...
  return (
    <Card title="Bloque 4 — CTA con stats card" subtitle="Sección oscura con texto + botón a la izquierda y tarjeta de 3 stats (la del medio destacada en rojo) + foto al final.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Field label="Badge" required>
      ...
      <span style={fieldLabel}>Chips rojos {cta.chips.length > 0 && `(${cta.chips.length})`}</span>
      <div className="flex flex-col gap-2">
        {cta.chips.map((c, i) => (
          <div key={i} className="flex gap-2 items-center">
    ...
    <div className="flex items-center justify-between gap-3 px-5 py-3 flex-wrap sticky top-0 z-10" style={{ background: "#FFFFFF", border: "1px solid #E8E4DD", borderRadius: 12 }}>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          style={{ height: 36, background: "#1A2B4A", color: "#FFFFFF", border: "none", fontSize: 14, fontWeight: 600, cursor: isPending ? "wait" : "pointer", opacity: isPending ? 0.7 : 1 }}
        >
