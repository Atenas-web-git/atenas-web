...
export async function POST(req: NextRequest) {
  try {
    ...
    // Si el slug del servicio configura un `destinatarioEmail` específico, lo usamos
    // como override del preset (caso "Quejas y sugerencias" tiene un destinatario
    // editable desde el editor de plantilla K). Si no, el preset.notifyTo aplica.
        ...
        <div style="background: #ffffff; padding: 32px; border: 1px solid #e8e4df; border-top: none; border-radius: 0 0 8px 8px;">
          <table style="width: 100%; border-collapse: collapse;">
            <tr style="border-bottom: 1px solid #f0ece7;">
              ...
              <td style="padding: 12px 0; font-size: 14px; font-weight: 600; color: #1A2B4A;">${s.nombre}</td>
            </tr>
            <tr style="border-bottom: 1px solid #f0ece7;">
            ...
            <tr style="border-bottom: 1px solid #f0ece7;">
              ...
              <td style="padding: 12px 0; font-size: 14px; font-weight: 700; color: #9e1915;">${s.tipo}</td>
            </tr>
            <tr>
              ...
              <td style="padding: 12px 0; font-size: 14px; color: #1A2B4A; line-height: 1.6;">${s.descripcion}</td>
            </tr>
          </table>
        </div>
    ...
    await sendEmail({
      ...
      subject,
      html,
      context: "POST /api/quejas (interno)",
    }).catch(() => {});
    await sendFormConfirmation({
      ...
      context: "POST /api/quejas (confirmación)",
    }).catch(() => {});
